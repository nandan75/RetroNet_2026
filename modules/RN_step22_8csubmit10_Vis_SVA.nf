process RN_step23_8csubmit10_Vis_SVA {
  //errorStrategy 'ignore' // Ignore errors
  tag "RN_step23_8csubmit10_Vis_SVA : ${sample_id}"
...
\${outpath}/\${sub}/script/8c_submit10_GPU.sh \\
	-o \${outpath} -j \${sub} -m ${params.masterpath} -v \${ver} -p \${partition_name} -g \${hg} -z \${gpu_partitio}n -c \${cont} -x \${cutoff} -t SVA -f SVA
